                ...
                if not self._finished[idx]:
                    ...
                    while len(q) > 1:
                        ...
                        if q or not check_convergence:
                            q.append(train(False, False, *data, **self._clf_params[idx]))
                        elif not q:
                            sv, sl, self._clf[idx] = compss_wait_on(train(True, False, *data, **self._clf_params[idx]))
                            q.append((sv, sl))
...
def merge(has_duplicates, *args):
    if issparse(args[0][0]):
        ...
        return merge_dense(has_duplicates, *args)


def merge_dense(has_duplicates, *args):
    sv1, sl1 = args[0]
