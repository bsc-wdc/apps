...
__author__ = ['Sandra Corella <sandra.corella@bsc.es>',
              'Sergio Rodriguez <sergio.rodriguez@bsc.es>'
             ]
...
def partial_sum(XP, clusters, ind):
    ...
    for i in clusters:
        ...
        dic[i] = (len(p_idx), np.sum(XP.content[p_idx], axis=0))
    return dic
...
def genFragment(numv, dim, k, seed, mode='random'):
    if mode == 'gauss':
        ret =  PSCO(init_board_gauss(numv, dim, k, seed))
    else:
        ret = PSCO(init_board_random(numv, dim, seed))
    ret.make_persistent()
    return ret

def kmeans_frag(numV, k, dim, epsilon, maxIterations, numFrag, seed = 5):
    ...
    while not has_converged(mu, oldmu, epsilon, n, maxIterations):
        ...
        while len(mu) < k:
            ...
            mu.append(X[indF].get_content()[indP])
        n += 1
