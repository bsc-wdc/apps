...
def kmeans_frag(X, k, epsilon, maxIterations, numFrag):
    ...
    while not has_converged(mu, oldmu, epsilon, n, maxIterations):
        oldmu = mu
        clusters = [cluster_points_partial(X[f * size:f * size + size],
                                          mu, f * size) for f in range(numFrag)]
        partialResult = [partial_sum(X[f * size:f * size + size],
                                 clusters[f], f * size) for f in range(numFrag)]

        partialResult = compss_wait_on(partialResult)
        precenters = reduceCenters(partialResult)
                     # merge(partialResult) seria mas bonito

        mu = [precenters[c][1] / precenters[c][0] for c in precenters]
        print mu
        n += 1
    return n
