...
def generateStringData(numRecords, uniqueKeys, keyLength, uniqueValues, valueLength, numPartitions, randomSeed, storageLocation, hashFunction):
    ...
    for i in range(numPartitions):
        data.append(map(lambda (k, v): (paddedString(k, keyLength, hashFunction), paddedString(v, valueLength, hashFunction)), ints[i]))
...
if __name__ == "__main__":
    ...
    hashFunction = False
    for i in ints:
        strings = map(lambda (k, v): (paddedString(k, keyLength, hashFunction), paddedString(v, valueLength, hashFunction)), i)
        print strings
    '''
    #out_path = "/gpfs/projects/bsc19/COMPSs_APPS/sortByKey/python/1.0_sortByKey/out"
    #print generateStringData(10, 10, 5, 10, 5, 2, 8, out_path, False)
