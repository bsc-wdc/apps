...
def generateRanges():
    ...
    # Init ranges
    rangeSize = (range_max - range_min) / numBuckets
    ranges = []
    # ranges = [(r, r + rangeSize) for r in range(min, max, rangeSize)] # Does not control the last range
    last = 0
    rangPos = 0
    for r in range(range_min, range_max, rangeSize):
        if r + rangPos + rangeSize < range_max:
            ranges.append((r + rangPos, r + rangPos + rangeSize))
            last = r + rangPos + rangeSize
            rangPos += 1
    if last < range_max:
        # There is a smaller range at the end
        ranges.append((last, range_max))
    return ranges
    '''
...
def terasort(numFragments, numEntries, numBuckets, seed):
    ...
    for d in dataset:
        fragmentBuckets = filterFragment(d, ranges)
        for i in range(numBuckets):
