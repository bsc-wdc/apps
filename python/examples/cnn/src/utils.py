...
class DataSet(object):
  ...
  def next_batch(self, batch_size, fake_data=False):
    ...
    if fake_data:
      ...
      if self.one_hot:
        ...
        fake_label = 0
      return [fake_image for _ in xrange(0, batch_size)], [
          fake_label for _ in xrange(0, batch_size)
      ]
  ...
  def get_batch(self, offset, batch_size):
      """Return the next `batch_size` examples from the given offset onwards."""

      start = offset
      self._index_in_epoch = offset + batch_size
      if self._index_in_epoch > self._num_examples:
          # Finished epoch
          self._epochs_completed += 1
          # Shuffle the data
