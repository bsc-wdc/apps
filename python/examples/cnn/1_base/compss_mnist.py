...
flags.DEFINE_string('checkpoint_dir', "models/", "Directory to load/store model checkpoint")
flags.DEFINE_string('checkpoint_file', "models/advanced_mnist.ckpt", "File to load/store model checkpoint")
flags.DEFINE_integer('batch_size', 50, "Number of elements used in each batch")
flags.DEFINE_integer('max_iterations', 200, "Max iterations to be performed per model")
flags.DEFINE_integer('task_batch', 100, "Number of iterations to be performed per task")
...
@task( returns=dict )
def do_training_step(model, iteration, task_iterations, batch_size, train_images, train_labels):


    from time import time
    from compssml.tf.utils import DataSet
    ...
    if iteration == 0:
	    ...
	    os.remove("%s.meta" % model['path'])
            print ("Model %s correctly loaded i:%s" % (model['path'], iteration))
        except Exception as e:
            print("No checkpoint for model %s found in iter %s\n%s" % (model['path'], iteration, e))
            sys.exit(1)
    ...
    print("Loop\n Avg: %s\n Max: %s\n Min: %s" % (step_times.mean(), step_times.max(), step_times.min()))
    print("Loop time %s" % (time()-t1))
    save_path = saver.save(sess, "%s_%s" % (model['base_path'], iteration))

    model['path'] = save_path
    
    training_accuracy = accuracy.eval(session=sess, feed_dict={x: batch[0], y_:batch[1], keep_prob: 1.0})
...
def do_testing(model, test_images, test_labels):
    ...
    test_accuracy = accuracy.eval(session=sess, feed_dict={x: test.images, y_: test.labels, keep_prob: 1.0})
...
def continue_exploration(model):
    ...
    print("Model, neuron number: %d, learning rate: %f, train accuracy: %f, test accuracy: %f, previous test: %f" % (n,l,t,test,prev))
    if test >= prev:
        return True
    else:
        return False
...
def init_models(base_dir, parametrizations):
    ...
    for n, l in parametrizations:
        path = "%s/%scompss_model_%s_%s_%s.ckpt" % (base_dir, FLAGS.checkpoint_dir, n, l, id)
        model = dict()
        model['path'] = path
...
def write_results(models, test_accuracies):
    ...
    with open('%s.%s' % (name, v), 'w') as f:
        ...
        for i in range(len(models)):
            ...
            print(" - Model [%s, %s]: %s" % (neurons, learning_rate, test_accuracies[i]))
