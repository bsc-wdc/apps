...
public class Sort {
    ...
    private static boolean READ_ENTIRE;
    ...
    public static void main(String args[]) {
        // Get parameters
        if (args.length != 3) {
            System.out.println("[ERROR] Usage: Sort <DATA_FOLDER> <OUTPUT_FOLDER> <READ_ENTIRE_FILE>");
            System.exit(-1);
        }
        DATA_FOLDER = args[0];
        ...
        READ_ENTIRE = Boolean.parseBoolean(args[2]);
        System.out.println("[LOG] READ_ENTIRE_FILE = " + READ_ENTIRE);
    ...
    private void run() {
        ...
        for (int i = 0; i < NUM_FRAGMENTS; ++i) {
            ...
            for (int j = 0; j < NUM_RANGES; ++j) {
                ranges[i][j] = SortImpl.getFilteredFragment(filePaths[i], start, end, READ_ENTIRE);
                start += step;
                end += step;
            }
        }
