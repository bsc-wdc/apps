...
public class Blast {
    ...
    public static void main(String args[]) throws Exception {
        ...
        try {
            ...
            // All-to-One merge
            assemblyPartitions(partialOutputs, outputFileName, temporaryDir);

            // ============================ Tree-based merge
            // ===================================
            /*
             * try{ //Final Assembly process -> Merge 2 by 2 int neighbor=1; while (neighbor<partialOutputs.size()){ for
             * (int result=0; result<partialOutputs.size(); result+=2*neighbor){ if (result+neighbor <
             * partialOutputs.size()){ BlastImpl.assemblyPartitions(partialOutputs.get(result),
             * partialOutputs.get(result+neighbor)); if(debug) System.out.println
             * (" - Merging files -> "+partialOutputs.get(result)+ " and "+partialOutputs.get(result+neighbor));
             * lastMerge = partialOutputs.get(result); } } neighbor*=2; } } catch (Exception e){ System.out.println(
             * "Error assembling partial results to final result file."); e.printStackTrace(); }
             * 
             * 
             ...
             * 
             * copyFile(fis, new File(outputFileName)); fis.close();
             * 
             * 
             * //Cleaning up partial results CleanUp();
             */
            // ==================================================================================
    ...
    private static void assemblyPartitions(List<String> partialOutputs, String outputFileName, String temporaryDir) {
        String line = null;
        Long startAssemblyTime = System.currentTimeMillis();

        try {
            // Cleaning intermediate sequence input files
            for (int i = 0; i < partialOutputs.size(); i++) {
                File fSeq = new File(temporaryDir + "seqFile" + i + ".sqf");
                fSeq.delete();
            }
            ...
            for (int i = 0; i < partialOutputs.size(); i++) {
                BufferedReader bf = new BufferedReader(new FileReader(partialOutputs.get(i)));
                if (debug) {
                    System.out.println(
                            " - Assembling partial output -> " + partialOutputs.get(i) + " to final output file -> " + outputFileName);
                }
                ...
                bf.close();
                File partOut = new File(partialOutputs.get(i));
                partOut.delete();
        ...
        Long stopAssemblyTime = System.currentTimeMillis();
        Long assemblyTime = (stopAssemblyTime - startAssemblyTime) / 1000;
        System.out.println("- Sequences assembled in " + assemblyTime + " seconds \n");
    }
