...
void Block::multiplyGPU(Block block1, Block block2)
{
	...
	//Array to store pointers to each partitions array
	...
	//Multiply the block with GPUs

	for (int i = 0; i < nPartitions; i++){
		for (int j = 0; j < nPartitions; j++){
			for (int k = 0; k < nPartitions; k++){
				Muld(partitionsA[i*nPartitions+k],partitionsB[k*nPartitions+j],M_part,M_part,partitionsC[i*nPartitions+j],M_part);				
			}
		}
	}
	...
	//transform array C into vector of vectors
	for (int i=0; i<M; i++)
	{
		for (int j=0; j<M; j++)
		{	
